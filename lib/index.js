...
function telemetryForSkill(store, cand) {
  ...
  if (samples.after.length >= 3) {
    // 相对恶化（有基线时）或绝对红线（无基线但失败率≥50%/纠错率≥50% 且样本≥3）
    const worseFail = (fa && fa.rate !== null && fb && fb.rate !== null && fa.rate > fb.rate * 1.2) || (fa && fa.rate !== null && fa.rate >= 0.5 && samples.after.length >= 3)
    const worseCorr = (ca && ca.rate !== null && cb && cb.rate !== null && ca.rate > cb.rate * 1.2) || (ca && ca.rate !== null && ca.rate >= 0.5 && samples.after.length >= 3)
...
function refreshTelemetry(store, opts = {}) {
  const db = loadCandidates(store.root)
  for (const cand of db.candidates) {
    ...
    if (cand.telemetry.verdict === 'deprecated' && cand.status === 'promoted') {
      let rollbackNote = ''
      if (opts.autoRollback !== false && opts.skillsDir) {
        const dst = rollbackSkill(opts.skillsDir, cand.name)
        if (dst && !String(dst).startsWith('ROLLBACK_FAILED')) {
          rollbackNote = '；已自动回滚至 ' + dst
        } else if (dst && String(dst).startsWith('ROLLBACK_FAILED')) {
          rollbackNote = '；自动回滚失败: ' + dst
        }
      }
      cand.status = 'deprecated'
      cand.decidedAt = Date.now()
      if (!String(cand.note || '').includes('telemetry 自动降级')) {
        cand.note = (cand.note ? cand.note + '; ' : '') + 'telemetry 自动降级: failRate ' + fmtRate(cand.telemetry.failRateBefore) + '→' + fmtRate(cand.telemetry.failRateAfter) + rollbackNote
      }
      appendEvolutionLog(store.root, { event: 'auto-rollback', name: cand.name, verdict: 'deprecated', note: cand.note })
    }
  }
...
// B0：决策日志——promote/reject/deprecate/rollback 全部追加进 evolution.log.jsonl
function evolutionLogFile(root) { return join(root, 'evolution.log.jsonl') }

function appendEvolutionLog(root, entry) {
  try {
    appendFileSync(evolutionLogFile(root), JSON.stringify({ at: Date.now(), ...entry }) + '\n')
  } catch { /* 日志失败不影响主流程 */ }
}

// B1：canary 冒烟门——写入后读回校验 frontmatter 可解析、含 name/description、无密钥
function canarySkillFile(file, name) {
  try {
    const text = readFileSync(file, 'utf8')
    if (!/^---$/m.test(text)) return 'frontmatter 分隔符缺失'
    if (!text.includes('name: ' + name)) return 'frontmatter 缺少 name: ' + name
    if (!/^description: /m.test(text)) return 'frontmatter 缺少 description'
    if (/(sk-[A-Za-z0-9]{16,})/.test(text)) return 'canary 发现疑似密钥'
    return null
  } catch (err) {
    return 'canary 读回失败: ' + ((err && err.message) || err)
  }
}

// B3：自动回滚——把已晋升的 skill 文件移入 skills-archive/（skill provider 不再发现，可恢复）
function rollbackSkill(skillsDir, name) {
  try {
    const src = skillFileFor(skillsDir, name)
    if (!existsSync(src)) return null
    const archiveDir = join(dirname(skillsDir), 'skills-archive')
    mkdirSync(archiveDir, { recursive: true })
    const dst = join(archiveDir, name + '.md')
    renameSync(src, dst)
    return dst
  } catch (err) {
    return 'ROLLBACK_FAILED: ' + ((err && err.message) || err)
  }
}
...
export function apply(ctx, rawConfig = {}) {
  ...
  ctx.tools.register({
    ...
    async execute(args, exec) {
      ...
      try { refreshTelemetry(store, { skillsDir: config.skillsDir || join(dshHome(), 'skills'), autoRollback: config.autoRollback }) } catch (err) { log('telemetry refresh failed:', err && err.message) }
  ...
  ctx.tools.register({
    ...
    async execute(args) {
      ...
      if (config.requireJudge) {
        ...
        const consistent = !config.judgeConsistency || (ev.includes(name) || (candidate.sourceTaskType && ev.includes(candidate.sourceTaskType)))
        if (jv !== 'pass' || ev.length < 20 || (config.judgeConsistency && !consistent)) {
          cand.status = 'rejected'
          cand.decidedAt = Date.now()
          cand.judge = judge || null
          cand.note = 'judge 门拒绝: ' + (consistent ? '需要 verdict=pass 且 evidence≥20 字符' : '裁判证据未引用候选名/类型，疑似模板化裁判（B2）')
          saveCandidates(root, db)
          return { ok: false, skillName: name, issues: [consistent ? 'judge 门拒绝：请提供独立裁判意见（judge.verdict=pass + judge.evidence≥20 字符，含合格理由与风险）' : 'judge 门拒绝（B2）：裁判证据未引用候选名或 taskType，疑似模板化裁判，请重写具体证据'] }
        }
      }
      ...
      const canaryIssue = config.skillGate ? canarySkillFile(path, name) : null
      if (canaryIssue) {
        try { renameSync(path, join(dirname(path), '.failed-' + name + '.md')) } catch {}
        cand.status = 'rejected'
        cand.decidedAt = Date.now()
        cand.note = 'canary 冒烟失败: ' + canaryIssue
        saveCandidates(root, db)
        if (config.evolutionLog) appendEvolutionLog(root, { event: 'promote-canary-failed', name, issue: canaryIssue })
        return { ok: false, skillName: name, issues: ['canary 冒烟失败（B1）：' + canaryIssue] }
      }
      ...
      saveCandidates(root, db)
      if (config.evolutionLog) appendEvolutionLog(root, { event: 'promote', name, path, judge: judge && judge.verdict, probe: probe && probe.outcome })
  ...
  ctx.tools.register({
    ...
    async execute(args) {
      const db = refreshTelemetry(store, { skillsDir: config.skillsDir || join(dshHome(), 'skills'), autoRollback: config.autoRollback })
  ...
  ctx.tools.register({
    ...
    async execute(args) {
      ...
      saveCandidates(root, db)
      if (config.evolutionLog) appendEvolutionLog(root, { event: 'reject', name: cand.name, status: cand.status, reason: (args && args.reason) || '' })
...
function handleRetroHttp(root, req, res, skillsDir) {
  ...
  try {
    if (req.method === 'GET' && url.pathname === '/retro/state') {
      ...
      let evolutionLogSize = 0
      try { evolutionLogSize = readFileSync(evolutionLogFile(root), 'utf8').split('\n').filter((l) => l.trim()).length } catch {}
      let archiveCount = 0
      if (skillsDir) { try { archiveCount = readdirSync(join(dirname(skillsDir), 'skills-archive')).filter((f) => f.endsWith('.md')).length } catch {} }
      send(200, { ok: true, candidates: list, counts, turnFiles, retroCount, storageRoot: root, evolutionLogSize, archiveCount })
      return
    }
  ...
  ctx.inject(['webServer'], (httpCtx) => {
    httpCtx.effect(() => httpCtx.webServer.register({
      kind: 'prefix',
      path: '/retro',
      handler: (req, res) => handleRetroHttp(root, req, res, config.skillsDir || join(dshHome(), 'skills')),
    }), 'dsh-retro: webserver')
  })
