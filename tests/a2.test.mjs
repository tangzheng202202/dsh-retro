...
test('A2: skill_promote 成功 → 写技能文件 + 记账 promoted', async () => {
  ...
  apply(ctx, { enabled: true, storageDir: dir, skillsDir: join(dir, 'skills'), inbox: false })
  const promote = findTool(ctx, 'skill_promote')
  const r = await promote.execute({ name: 'dsh-plugin-zero-dep', judge: { verdict: 'pass', evidence: 'dsh-plugin-zero-dep 候选完整且通过机械检查，指令可执行，无密钥风险', risks: '无' } })
...
test('A2: skill_promote 冲突保护与 overwrite', async () => {
  ...
  apply(ctx, { enabled: true, storageDir: dir, skillsDir: join(dir, 'skills'), inbox: false })
  const promote = findTool(ctx, 'skill_promote')
  await promote.execute({ name: 'dsh-plugin-zero-dep', judge: { verdict: 'pass', evidence: 'dsh-plugin-zero-dep 候选完整且通过机械检查，指令可执行，无密钥风险', risks: '无' } })
  // 已 promoted → 拒绝重复
  const again = await promote.execute({ name: 'dsh-plugin-zero-dep', judge: { verdict: 'pass', evidence: 'dsh-plugin-zero-dep 候选完整且通过机械检查，指令可执行，无密钥风险', risks: '无' } })
